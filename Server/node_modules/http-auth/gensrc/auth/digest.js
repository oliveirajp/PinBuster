...
(function() {
  ...
  Digest = (function(superClass) {
    extend(Digest, superClass);

    function Digest(options1, checker1) {
    ...
    Digest.prototype.processLine = function(line) {
      var hash, realm, ref, username;
      ref = line.split(":"), username = ref[0], realm = ref[1], hash = ref[2];
    ...
    Digest.prototype.removeNonces = function(noncesToRemove) {
      var index, j, len1, nonce, results;
      results = [];
      for (j = 0, len1 = noncesToRemove.length; j < len1; j++) {
        nonce = noncesToRemove[j];
        index = this.nonces.indexOf(nonce);
        if (index !== -1) {
          results.push(this.nonces.splice(index, 1));
        } else {
          results.push(void 0);
        }
      }
    ...
    Digest.prototype.validateNonce = function(nonce) {
      var found, j, len1, noncesToRemove, now, ref, serverNonce;
      now = Date.now();
      noncesToRemove = [];
      ref = this.nonces;
      for (j = 0, len1 = ref.length; j < len1; j++) {
